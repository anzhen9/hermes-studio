...
constexpr char kMcuFirmwareVersion[] = "v2";
constexpr char kMcuFirmwareManifestPath[] = "/api/hermes/mcu/sparkbot/firmware/v2/manifest";
...
constexpr int kPinI2cSda = 4;
constexpr int kPinI2cScl = 5;
// I2S audio (ES8311 codec)
constexpr int kPinI2sBck = 39;
constexpr int kPinI2sWs = 41;
constexpr int kPinI2sDout = 42;
constexpr int kPinI2sDin = 40;
constexpr int kPinI2sMck = 45;
// Boot button
constexpr int kPinBoot = 0;
// ST7789 LCD (SPI3)
constexpr int kPinLcdMosi = 47;
constexpr int kPinLcdClk = 21;
...
constexpr uint32_t kActivePetCacheSaveDelayMs = 120000;
...
constexpr uint32_t kActivePetSpriteTimeoutMs = 90000;
...
constexpr int kMcuAudioDefaultSampleRate = kAudioSampleRate;
...
constexpr uint16_t kColorAccent = 0x07FF;  // Cyan
constexpr uint16_t kColorGood = 0x07E0;    // Green
constexpr uint16_t kColorWarn = 0xF800;    // Red
constexpr uint16_t kColorThink = 0xFFE0;   // Yellow
constexpr uint16_t kColorMuted = 0x4208;   // Dark gray
...
bool petCacheSavePending = false;
uint32_t nextPetSwitchAttemptAtMs = 0;
uint32_t nextPetCacheSaveAtMs = 0;
...
LcdMode lcdMode = LcdMode::Boot;

// ---------------------------------------------------------------------------
// Utility helpers
// ---------------------------------------------------------------------------

String fitLcdText(String value, size_t maxLen) {
  value.trim();
  value.toUpperCase();
...
// ---------------------------------------------------------------------------
// ST7789 LCD driver (SPI, 240x240 RGB565)
// ---------------------------------------------------------------------------

void lcdCommand(uint8_t cmd) {
  digitalWrite(kPinLcdDc, LOW);
  digitalWrite(kPinLcdCs, LOW);
  lcdSpi.write(cmd);
  digitalWrite(kPinLcdCs, HIGH);
}

void lcdData(const uint8_t *data, size_t len) {
  digitalWrite(kPinLcdDc, HIGH);
  digitalWrite(kPinLcdCs, LOW);
  lcdSpi.writeBytes(const_cast<uint8_t *>(data), len);
  digitalWrite(kPinLcdCs, HIGH);
}

void lcdDataByte(uint8_t value) {
  lcdData(&value, 1);
}
...
void lcdSetWindow(int x, int y, int w, int h) {
  ...
  int y2 = y + h - 1;
  lcdCommand(0x2A);
  uint8_t col[4] = {static_cast<uint8_t>(x >> 8), static_cast<uint8_t>(x & 0xFF),
                    static_cast<uint8_t>(x2 >> 8), static_cast<uint8_t>(x2 & 0xFF)};
  lcdData(col, 4);
  lcdCommand(0x2B);
  uint8_t row[4] = {static_cast<uint8_t>(y >> 8), static_cast<uint8_t>(y & 0xFF),
...
bool initLcdHardware() {
  ...
  pinMode(kPinLcdCs, OUTPUT);
  digitalWrite(kPinLcdCs, HIGH);

  lcdSpi.begin(kPinLcdClk, -1, kPinLcdMosi, kPinLcdCs);
  lcdSpi.setFrequency(kSpiFrequency);
  lcdSpi.setBitOrder(MSBFIRST);
  ...
  lcdCommand(0x3A);  // Color mode 16-bit
  lcdDataByte(0x55);

  lcdCommand(0x36);  // Memory data access control
  lcdDataByte(0x00);  // Normal scan, RGB

  lcdCommand(0xB2);  // Porch setting
  uint8_t porch[] = {0x0C, 0x0C, 0x00, 0x33, 0x33};
  lcdData(porch, sizeof(porch));

  lcdCommand(0xB7);  // Gate control
  lcdDataByte(0x35);

  lcdCommand(0xBB);  // VCOM setting
  lcdDataByte(0x19);

  lcdCommand(0xC0);  // LCM control
  lcdDataByte(0x2C);

  lcdCommand(0xC2);  // VDV/VRH enable
  lcdDataByte(0x01);

  lcdCommand(0xC3);  // VRH set
  lcdDataByte(0x12);

  lcdCommand(0xC4);  // VDV set
  lcdDataByte(0x20);

  lcdCommand(0xC6);  // Frame rate control
  lcdDataByte(0x0F);

  lcdCommand(0xD0);  // Power control
  uint8_t pwctl[] = {0xA4, 0xA1};
  lcdData(pwctl, sizeof(pwctl));

  lcdCommand(0xE0);  // Positive gamma
  uint8_t pgam[] = {0xD0, 0x04, 0x0D, 0x11, 0x13, 0x2B, 0x3F, 0x54,
                    0x4C, 0x18, 0x0D, 0x0B, 0x1F, 0x23};
  lcdData(pgam, sizeof(pgam));

  lcdCommand(0xE1);  // Negative gamma
  uint8_t ngam[] = {0xD0, 0x04, 0x0C, 0x11, 0x13, 0x2C, 0x3F, 0x44,
                    0x51, 0x2F, 0x1F, 0x1F, 0x20, 0x23};
  lcdData(ngam, sizeof(ngam));

  lcdCommand(0x21);  // Display inversion on
...
// ---------------------------------------------------------------------------
// LCD framebuffer drawing primitives (RGB565 in PSRAM)
// ---------------------------------------------------------------------------

void lcdClearBuffer() {
  if (!lcdBuffer) return;
  size_t total = static_cast<size_t>(kLcdWidth) * kLcdHeight;
  for (size_t i = 0; i < total; ++i) lcdBuffer[i] = kColorBg;
}

void lcdSetPixel(int x, int y, uint16_t color) {
  if (x < 0 || x >= kLcdWidth || y < 0 || y >= kLcdHeight) return;
  lcdBuffer[y * kLcdWidth + x] = color;
}

void lcdDrawBox(int x, int y, int w, int h, uint16_t color) {
  for (int yy = y; yy < y + h; ++yy) {
    for (int xx = x; xx < x + w; ++xx) {
      lcdSetPixel(xx, yy, color);
    }
  }
}

void lcdDrawHLine(int x, int y, int w, uint16_t color) {
  lcdDrawBox(x, y, w, 1, color);
}

void lcdDrawVLine(int x, int y, int h, uint16_t color) {
  lcdDrawBox(x, y, 1, h, color);
}

void lcdDrawFrame(int x, int y, int w, int h, uint16_t color) {
...
void lcdDrawSoftBox(int x, int y, int w, int h, int radius, uint16_t color) {
  ...
  for (int yy = 0; yy < h; ++yy) {
    ...
    inset = min(inset, w / 2);
    lcdDrawBox(x + inset, y + yy, max(0, w - inset * 2), 1, color);
  }
}
...
void lcdDrawChar(int x, int y, char c, uint8_t scale, uint16_t color) {
  const uint8_t *glyph = glyph5x7(c);
  for (uint8_t col = 0; col < 5; ++col) {
    for (uint8_t row = 0; row < 7; ++row) {
      if (glyph[col] & (1 << row)) {
        lcdDrawBox(x + col * scale, y + row * scale, scale, scale, color);
      }
    }
  }
}

void lcdDrawText(int x, int y, const String &text, uint8_t scale, uint16_t color) {
  for (size_t i = 0; i < text.length(); ++i) {
    lcdDrawChar(x + i * 6 * scale, y, text[i], scale, color);
  }
}

int lcdTextWidth(const String &text, uint8_t scale) {
  return text.length() == 0 ? 0 : static_cast<int>(text.length()) * 6 * scale - scale;
}

void lcdDrawCenteredText(int y, const String &text, uint8_t scale, uint16_t color) {
  int x = (kLcdWidth - lcdTextWidth(text, scale)) / 2;
...
void lcdDrawScrollingText(int y, String text, uint8_t scale, uint16_t color) {
  ...
  int width = lcdTextWidth(text, scale);
  if (width <= kLcdWidth) {
    lcdDrawCenteredText(y, text, scale, color);
    return;
  }
  ...
  lcdDrawText(-offset, y, text, scale, color);
  lcdDrawText(width + gap - offset, y, text, scale, color);
}
...
void drawWifiGlyph(uint8_t x, uint8_t y, uint16_t color) {
  ...
  for (uint8_t i = 0; i < 4; ++i) {
    ...
    uint8_t by = y + 18 - h;
    if (i < bars) {
      lcdDrawBox(bx, by, 5, h, color);
    } else {
      lcdDrawFrame(bx, by, 5, h, kColorMuted);
    }
  }
}
...
void drawEye(int x, int y, bool blink, bool thinking, bool error) {
  const int w = 56;
  int h = thinking ? 44 : 56;
  if (error) {
    lcdDrawHLine(x + 10, y + h / 2 - 6, w - 20, kColorWarn);
    lcdDrawHLine(x + 10, y + h / 2 + 4, w - 20, kColorWarn);
    return;
  }
  if (blink) {
    lcdDrawSoftBox(x + 4, y + h / 2 - 3, w - 8, 6, 6, kColorFg);
    return;
  }
  lcdDrawSoftBox(x, y, w, h, 12, kColorAccent);
  int phase = static_cast<int>((millis() / 1400UL) % 5);
  int lookX = thinking ? 0 : (phase == 1 ? -6 : (phase == 3 ? 6 : 0));
  lcdDrawSoftBox(x + w / 2 - 10 + lookX, y + h / 2 - 10, 20, 20, 4, kColorBg);
}
...
void drawInteractionFrame() {
  ...
  lcdDrawCenteredText(50, fitLcdText(title, 12), 3, titleColor);
  ...
  if (detail.length() > 20 || mcuInteractionStatus == F("failed")) {
    lcdDrawScrollingText(120, detail, 2, kColorFg);
  } else {
    lcdDrawCenteredText(120, fitLcdText(detail, 20), 2, kColorFg);
  }
  ...
  if (mcuAudioPlaying && mcuAudioDurationMs > 0) {
    ...
    lcdDrawFrame(20, 180, 200, 8, kColorMuted);
    uint8_t filled = static_cast<uint8_t>((progress * 192) / 100);
    if (filled > 0) lcdDrawBox(24, 182, filled, 4, kColorAccent);
  } else {
    String queue = String(F("QUEUE ")) + mcuAudioCount;
    lcdDrawCenteredText(185, queue, 2, kColorMuted);
  }
}
...
bool lcdFlush() {
  ...
  digitalWrite(kPinLcdDc, HIGH);
  digitalWrite(kPinLcdCs, LOW);
  static uint8_t sendBuf[kLcdWidth * 2];
  for (int y = 0; y < kLcdHeight; ++y) {
    uint16_t *row = lcdBuffer + y * kLcdWidth;
    for (int x = 0; x < kLcdWidth; ++x) {
      sendBuf[x * 2] = static_cast<uint8_t>(row[x] >> 8);
      sendBuf[x * 2 + 1] = static_cast<uint8_t>(row[x] & 0xFF);
    }
    lcdSpi.writeBytes(sendBuf, kLcdWidth * 2);
  }
  digitalWrite(kPinLcdCs, HIGH);
  return true;
}
...
void drawLcdFrame() {
  ...
  bool thinking = lcdMode == LcdMode::Think;
  bool error = lcdMode == LcdMode::Error;
  ...
  if (thinking) {
    ...
    for (uint8_t i = 0; i < 3; ++i) {
      uint8_t h = 3 + ((phase + i) % 3) * 3;
      lcdDrawBox(110 + i * 8, 150 - h, 4, h, kColorThink);
    }
  } else if (lcdProgress > 0) {
    lcdDrawFrame(20, 180, 200, 8, kColorMuted);
    uint8_t filled = static_cast<uint8_t>((lcdProgress * 192) / 100);
    if (filled > 0) lcdDrawBox(24, 182, filled, 4, kColorAccent);
  }
  String status = lcdTitle;
  if (lcdHint.length() > 0 && lcdHint != lcdTitle) {
    status += F(" ");
    status += lcdHint;
  }
  lcdDrawScrollingText(200, status, 2, kColorFg);
}

void refreshLcd(bool force) {
  if (!lcdReady) return;
...
void setLcdStatus(LcdMode mode, const String &title, const String &hint, uint8_t progress = 0) {
  String nextTitle = fitLcdText(title, 10);
  String nextHint = fitLcdText(hint, 20);
  lcdMode = mode;
  lcdTitle = nextTitle;
  lcdHint = nextHint;
...
void initLcdDisplay() {
  lcdFound = initLcdHardware();
  if (!lcdFound) return;

  if (!lcdBuffer) {
    lcdBuffer = static_cast<uint16_t *>(ps_malloc(static_cast<size_t>(kLcdWidth) * kLcdHeight * sizeof(uint16_t)));
  }
  if (!lcdBuffer) {
    lcdReady = false;
    return;
  ...
  lcdClearBuffer();
  lcdReady = lcdFlush();
  if (lcdReady) setLcdStatus(LcdMode::Boot, F("BOOT"), F("LCD ONLINE"), 10);
}

// ---------------------------------------------------------------------------
// String / JSON / HTML helpers (identical to C3)
// ---------------------------------------------------------------------------
...
String deviceId() {
  ...
  mac.replace(":", "");
  mac.toLowerCase();
  return String(F("hstudio_esp32s3_")) + mac;
}
...
bool readPetSpriteResponseHeaders(WiFiClient &client, int *statusCode, int *contentLength, uint8_t *rowCount) {
  if (!statusCode || !contentLength || !rowCount) return false;
  String status = readHttpLine(client, kActivePetSpriteTimeoutMs);
  status.trim();
  ...
  *rowCount = 1;
  ...
  while (true) {
    String line = readHttpLine(client, kActivePetSpriteTimeoutMs);
    line.trim();
    ...
    } else if (name == F("x-hermes-image-rows")) {
...
// ---------------------------------------------------------------------------
// ES8311 codec + I2S audio (adapted pins for SparkBot)
// ---------------------------------------------------------------------------
...
bool configureI2sBus() {
  ...
  config.sample_rate = kVoiceInputSampleRate;
...
bool recordVoiceWav(uint8_t **outWav, size_t *outLen) {
  ...
  *outLen = 0;
  ...
  if (!i2sReady || !es8311Ready) {
    ...
    setLcdStatus(LcdMode::Error, F("AUDIO"), F("INPUT OFF"), 0);
    return false;
  }
  ...
  while (framesDone < maxFrames) {
    ...
    if (bytesRead == 0) {
      ++emptyReads;
      setLcdStatus(LcdMode::Think, F("LISTEN"), F("WAIT I2S"), 5);
      yield();
      continue;
    }
    ...
    uint32_t now = millis();
    if (progress != lastRecordProgress && now - lastRecordLcdAtMs >= 250) {
      lastRecordProgress = progress;
      lastRecordLcdAtMs = now;
      setLcdStatus(LcdMode::Think, F("LISTEN"), F("RECORDING"), progress);
    }
    yield();
  }
...
void scanWifiList() {
  ...
  setLcdStatus(LcdMode::Think, F("WIFI"), F("SCAN"), 25);
  WiFi.mode(WIFI_STA);
  WiFi.setSleep(false);
...
void sendWifiPage() {
  ...
  html += F("<section class='panel'><p class='meta'>HStudio ESP-SparkBot</p><h1>连接局域网 Wi-Fi</h1>");
  if (wifiReady) {
...
void sendStatusPage() {
  ...
  html += F("<section class='panel'><p class='meta'>HStudio ESP-SparkBot</p><h1>设备</h1><p class='lead'>");
  html += escapeHtml(WiFi.SSID());
  html += F(" · IP ");
  html += WiFi.localIP().toString();
...
void sendOtaUpdatingPage() {
  String html = pageStart(F("OTA"));
  html += F("<section class='panel'><p class='meta'>HStudio ESP-SparkBot</p><h1>OTA</h1><p class='lead'>固件正在更新</p>");
...
bool runMcuLogin(LanDevice &device, const String &account, const String &password) {
  ...
  setLcdStatus(LcdMode::Think, F("LOGIN"), F("MCU"), 40);
  HTTPClient http;
  http.setTimeout(kMcuLoginTimeoutMs);
  if (!http.begin(endpoint)) {
    rememberMcuLoginResult(-1, F("无法打开登录接口"));
    setLcdStatus(LcdMode::Error, F("LOGIN"), F("OPEN"), 0);
    return false;
  }

  http.addHeader(F("Content-Type"), F("application/json"));
  http.addHeader(F("X-Hermes-Device-Id"), deviceId());
  http.addHeader(F("X-Hermes-Device-Name"), F("HStudio ESP-SparkBot"));
  ...
  if (ok) {
    ...
    clearLoginProfiles();
    setLcdStatus(LcdMode::Error, F("LOGIN"), F("FAILED"), 0);
  }
  return ok;
}
...
// ---------------------------------------------------------------------------
// WebSocket / Socket.IO client (identical to C3)
// ---------------------------------------------------------------------------
...
bool playRecordedWav(uint8_t *wav, size_t wavLen) {
  ...
  setPowerAmp(true);
  es8311UpdateBits(0x31, 0x60, 0x00);
  setLcdStatus(LcdMode::Think, F("PLAY"), F("REC"), 0);
  ...
  while (remaining >= 2) {
    ...
    if (err != ESP_OK || written != bytesToWrite) {
      ...
      setLcdStatus(LcdMode::Error, F("PLAY"), F("FAIL"), 0);
      return false;
    }
...
void markMcuInteraction(const String &interactionId, const String &status, const String &text) {
  ...
  if (mcuInteractionStatus == F("failed")) {
    setLcdStatus(LcdMode::Error, F("ERROR"), F("DETAIL"), 0);
  } else if (lcdMode == LcdMode::Error) {
    setLcdStatus(LcdMode::Think, interactionStatusLabel(), F(""), 0);
  }
  lcdDirty = true;
}
...
void clearMcuSessionByButton() {
  ...
  if (!wifiReady || WiFi.status() != WL_CONNECTED) {
    setLcdStatus(LcdMode::Error, F("WIFI"), F("OFFLINE"), 0);
...
bool loadActivePetCache() {
  if (!openPetCachePartition()) {
    Serial.println(F("Pet cache restore skipped: partition unavailable"));
    return false;
  }

  PetCacheHeader header;
  if (esp_partition_read(petCachePartition, 0, &header, sizeof(header)) != ESP_OK) {
    Serial.println(F("Pet cache restore skipped: header read failed"));
    return false;
  }
  if (header.magic != kPetCacheMagic || header.version != kPetCacheVersion ||
    ...
    Serial.printf("Pet cache restore skipped: magic=%08lx version=%u header=%u profile=%s cached=%s slug=%s bytes=%lu\n",
                  static_cast<unsigned long>(header.magic), static_cast<unsigned>(header.version),
                  static_cast<unsigned>(header.headerBytes), selectedProfile.c_str(), header.profile, header.slug,
                  static_cast<unsigned long>(header.payloadBytes));
    return false;
  }
  ...
  if (!pixels || esp_partition_read(petCachePartition, kPetCacheHeaderBytes, pixels, header.payloadBytes) != ESP_OK) {
    ...
    Serial.println(F("Pet cache restore skipped: payload allocation or read failed"));
    return false;
  }
  if (esp_crc32_le(UINT32_MAX, reinterpret_cast<const uint8_t *>(pixels), header.payloadBytes) != header.payloadCrc32) {
    free(pixels);
    Serial.println(F("Pet cache restore skipped: payload CRC mismatch"));
    return false;
  }
...
bool saveActivePetCache() {
  if (!openPetCachePartition() || !activePetDisplay.spritePixels) return false;
  ...
  constexpr size_t kCacheEraseBlockBytes = 4096;
  constexpr size_t kCacheWriteChunkBytes = 32 * 1024;
  ...
  size_t totalBytes = kPetCacheHeaderBytes + payloadBytes;
  size_t eraseBytes = ((totalBytes + kCacheEraseBlockBytes - 1) / kCacheEraseBlockBytes) * kCacheEraseBlockBytes;
  if (eraseBytes > petCachePartition->size) return false;

  for (size_t offset = 0; offset < eraseBytes; offset += kCacheEraseBlockBytes) {
    if (esp_partition_erase_range(petCachePartition, offset, kCacheEraseBlockBytes) != ESP_OK) {
      Serial.printf("Pet cache erase failed offset=%u size=%u\n",
                    static_cast<unsigned>(offset), static_cast<unsigned>(kCacheEraseBlockBytes));
      return false;
    }
  ...
  const uint8_t *payload = reinterpret_cast<const uint8_t *>(activePetDisplay.spritePixels);
  for (size_t offset = 0; offset < payloadBytes; offset += kCacheWriteChunkBytes) {
    size_t chunkBytes = min(kCacheWriteChunkBytes, payloadBytes - offset);
    if (esp_partition_write(petCachePartition, kPetCacheHeaderBytes + offset, payload + offset, chunkBytes) != ESP_OK) {
      Serial.printf("Pet cache payload write failed offset=%u size=%u\n",
                    static_cast<unsigned>(offset), static_cast<unsigned>(chunkBytes));
      return false;
    }
    vTaskDelay(pdMS_TO_TICKS(1));
  }
  ...
  if (esp_partition_write(petCachePartition, 0, &header, sizeof(header)) != ESP_OK) {
    Serial.println(F("Pet cache header write failed"));
    return false;
  }
...
void requestActivePetRefresh(bool force) {
  if (!petSyncTaskHandle) {
    refreshActivePetDisplay(force, false);
    return;
  }
...
void activePetTask(void *) {
  for (;;) {
    ...
    if (switching) {
      ...
      refreshActivePetDisplay((work & kPetTaskWorkForceRefresh) != 0, false);
    }
...
void startActivePetTask() {
  ...
  Serial.println(F("Active pet task started core=1"));
}
...
bool refreshActivePetDisplay(bool force, bool saveCache) {
  ...
  if (!force && activePetDisplay.lastRefreshedAtMs > 0 && now - activePetDisplay.lastRefreshedAtMs < kActivePetRefreshIntervalMs) {
    if (activePetDisplay.available && (!activePetDisplay.spritePixels || activePetDisplay.loadedSpriteRevision != activePetDisplay.spritesheetRevision)) {
      refreshActivePetSprite(false, saveCache);
    }
    if (activePetDisplay.available) {
      refreshPetState();
    }
  ...
  if (!wifiReady || WiFi.status() != WL_CONNECTED || activeDeviceUrl.length() == 0 || mcuAuthToken.length() == 0 || selectedProfile.length() == 0) {
    if (force) {
      Serial.printf("Active pet refresh blocked wifi=%d url=%d token=%d profile=%s\n",
                    wifiReady && WiFi.status() == WL_CONNECTED ? 1 : 0, activeDeviceUrl.length() > 0 ? 1 : 0,
                    mcuAuthToken.length() > 0 ? 1 : 0, selectedProfile.c_str());
    }
  ...
  if (!http.begin(endpoint)) {
    Serial.printf("Active pet metadata begin failed endpoint=%s\n", endpoint.c_str());
    activePetDisplay.lastRefreshedAtMs = now;
    return false;
  }
  http.addHeader(F("Authorization"), String(F("Bearer ")) + mcuAuthToken);
  ...
  activePetDisplay.lastRefreshedAtMs = now;
  if (code < 200 || code >= 300) {
    Serial.printf("Active pet metadata HTTP %d profile=%s body=%s\n", code, selectedProfile.c_str(), body.c_str());
    return false;
  }
  ...
  if (petJson.length() == 0 || slug.length() == 0) {
    Serial.printf("Active pet metadata invalid body=%s\n", body.c_str());
    clearActivePetDisplay();
    return false;
  }
...
bool refreshActivePetSprite(bool force, bool saveCache) {
  ...
  if (endpoint.length() == 0) {
    Serial.println(F("Active pet sprite blocked: endpoint unavailable"));
    return false;
  }
  ...
  String path;
  uint16_t port = 80;
  if (!parseAudioUrl(endpoint, &scheme, &host, &port, &path)) {
    Serial.printf("Active pet sprite parse failed endpoint=%s\n", endpoint.c_str());
  ...
  client->setTimeout(kActivePetSpriteTimeoutMs / 1000);
  if (!client->connect(host.c_str(), port)) {
    Serial.printf("Active pet sprite connect failed host=%s port=%u\n", host.c_str(), port);
    activePetDisplay.lastSpriteRefreshAtMs = now;
    return false;
  }
  ...
  client->print(mcuAuthToken);
  client->print(F("\r\nX-Hermes-Profile: "));
  client->print(selectedProfile);
  ...
  int contentLength = -1;
  uint8_t rowCount = 1;
  bool headersOk = readPetSpriteResponseHeaders(*client, &code, &contentLength, &rowCount);
  uint32_t headersMs = millis() - requestStartedAt;
  if (!headersOk || code < 200 || code >= 300) {
    Serial.printf("Active pet sprite HTTP %d profile=%s headers=%lums\n",
                  code, selectedProfile.c_str(), static_cast<unsigned long>(headersMs));
    client->stop();
    activePetDisplay.lastSpriteRefreshAtMs = now;
    return false;
  }
  ...
  if (!pixels) {
    Serial.printf("Active pet sprite allocation failed bytes=%u free_psram=%u\n",
                  static_cast<unsigned>(expectedBytes), static_cast<unsigned>(ESP.getFreePsram()));
    client->stop();
    activePetDisplay.lastSpriteRefreshAtMs = now;
    return false;
  }
  ...
  while (readBytesTotal < expectedBytes) {
    ...
    if (available <= 0) {
      if (!client->connected()) break;
      if (millis() - downloadStartedAt >= kActivePetSpriteTimeoutMs) break;
      delay(2);
      if (wsReady) mcuSocketLoop();
      handleTouchButtons();
      refreshLcd();
    ...
    int bytesRead = client->readBytes(reinterpret_cast<uint8_t *>(pixels) + readBytesTotal, toRead);
  ...
  client->stop();
  uint32_t downloadMs = millis() - downloadStartedAt;

  if (readBytesTotal != expectedBytes) {
    Serial.printf("Active pet sprite incomplete expected=%u got=%u headers=%lums download=%lums\n",
                  static_cast<unsigned>(expectedBytes), static_cast<unsigned>(readBytesTotal),
                  static_cast<unsigned long>(headersMs), static_cast<unsigned long>(downloadMs));
    free(pixels);
    activePetDisplay.lastSpriteRefreshAtMs = now;
    return false;
  }
  ...
  if (saveCache) {
    ...
    nextPetCacheSaveAtMs = millis() + kActivePetCacheSaveDelayMs;
  }
  lcdDirty = true;
  Serial.printf("Active pet sprite ready slug=%s headers=%lums download=%lums bytes=%u\n",
                activePetDisplay.slug.c_str(), static_cast<unsigned long>(headersMs),
                static_cast<unsigned long>(downloadMs), static_cast<unsigned>(readBytesTotal));
  return true;
}
...
bool downloadAndApplyMcuFirmware(const String &url, const String &md5, int expectedSize) {
  ...
  setLcdStatus(LcdMode::Think, F("OTA"), F("UPDATE"), 10);
  ...
  setLcdStatus(LcdMode::Ready, F("OTA"), F("RESTART"), 100);
  delay(500);
  ESP.restart();
  return true;
}
...
bool recordAndBroadcastMcuVoiceStream(const String &interactionId, bool touchTriggered) {
  ...
  while (framesDone < maxFrames) {
    ...
    if (wakeWordVoiceTurn && loopNow - startedAt >= kVoiceRecordMs) {
      stopReason = "wake-window";
      break;
    }
    if (!wakeWordVoiceTurn && framesDone > 0 && loopNow - startedAt > kBootDebounceMs) {
...
void initializeTouchButtons() {
  touchSetConfig(0x1000, TOUCH_VOLT_LIM_L_0V5, TOUCH_VOLT_LIM_H_2V2);
...
void sendConnectSuccessPage(const String &ssid, const IPAddress &ip) {
  ...
  html += F("<section class='panel'><p class='meta'>HStudio ESP-SparkBot</p><h1>Wi-Fi 已连接</h1>");
...
void sendConnectFailedPage(const String &ssid) {
  ...
  html += F("<section class='panel'><p class='meta'>HStudio ESP-SparkBot</p><h1>Wi-Fi 连接失败</h1>");
...
bool connectWifiCredentials(const String &ssid, const String &pass, wifi_mode_t mode) {
  setLcdStatus(LcdMode::Think, F("WIFI"), F("CONNECT"), 25);
  WiFi.mode(mode);
  WiFi.setSleep(false);
  WiFi.disconnect(false, false);
  delay(100);
  ...
  if (wifiReady) {
    ...
    setLcdStatus(LcdMode::Error, F("WIFI"), F("FAILED"), 0);
    delay(500);
  }
  return wifiReady;
}
...
void saveWifi() {
  ...
  if (connectWifiCredentials(ssid, pass, WIFI_AP_STA)) {
    ...
    setLcdStatus(LcdMode::Think, F("WIFI"), F("RESTART"), 90);
    return;
  }
...
void startSetupAp(bool refreshScan = true) {
  ...
  setupApMode = ok;
  wifiReady = false;
  setLcdStatus(ok ? LcdMode::Ready : LcdMode::Error, ok ? F("SETUP") : F("AP ERR"),
...
void setup() {
  ...
  delay(300);
  Serial.println();
  Serial.println(F("HStudio WiFi setup firmware boot (ESP32-S3 ESP-SparkBot)"));
...
void loop() {
  ...
  refreshLcd();
  if (petCacheSavePending && !petSwitching && !petDisplayUpdating &&
      !mcuInteractionActive && !audioBusy && !mcuAudioPlaying &&
      static_cast<int32_t>(millis() - nextPetCacheSaveAtMs) >= 0) {
    Serial.println(F("Saving deferred pet cache"));
    if (saveActivePetCache()) {
      petCacheSavePending = false;
      nextPetCacheSaveAtMs = 0;
    }
  }
  ...
  if (wifiReady && WiFi.status() != WL_CONNECTED) {
    ...
    if (wifiDisconnectedSinceMs == 0) {
      ...
      setLcdStatus(LcdMode::Think, F("WIFI"), F("RECONNECT"), 60);
