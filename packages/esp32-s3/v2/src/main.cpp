...
#endif
// S3 语音板引脚（接线定义.md）：
//   I2C_SCL=39 I2C_SDA=40 I2S_MCK=38 I2S_BCK=16 I2S_WS=7 I2S_DO=4 I2S_DI=15
//   ES8311_PA=8 TP_KEY=5 LCD_SDA=1 LCD_SCL=18 LCD_DC=17 LCD_RESET=6 LCD_BL=2
// 屏幕引脚（TFT_MOSI/TFT_SCLK/TFT_DC/TFT_RST/TFT_BL）由 TFT_eSPI 编译宏提供
// （见 platformio.ini build_flags），不在此重复定义。
#ifndef HERMES_PIN_BATTERY_ADC
#define HERMES_PIN_BATTERY_ADC 0
#endif
// S3 板为外接电源，无电池分压 ADC。
#ifndef HERMES_ENABLE_BATTERY_ADC
#define HERMES_ENABLE_BATTERY_ADC 0
#endif
#ifndef HERMES_PIN_I2C_SDA
#define HERMES_PIN_I2C_SDA 40
#endif
#ifndef HERMES_PIN_I2C_SCL
#define HERMES_PIN_I2C_SCL 39
#endif
#ifndef HERMES_PIN_I2S_DOUT
#define HERMES_PIN_I2S_DOUT 4
#endif
#ifndef HERMES_PIN_I2S_WS
#define HERMES_PIN_I2S_WS 7
#endif
#ifndef HERMES_PIN_I2S_DIN
#define HERMES_PIN_I2S_DIN 15
#endif
#ifndef HERMES_PIN_I2S_BCK
#define HERMES_PIN_I2S_BCK 16
#endif
#ifndef HERMES_PIN_I2S_MCK
#define HERMES_PIN_I2S_MCK 38
#endif
// 用户键为弹簧触摸电极直连 IO5（板上无触摸芯片，digitalRead 方案无效），
// 走 S3 原生触摸外设测量电容变化，见下方 touchKeyPressed()。
#ifndef HERMES_PIN_BOOT
#define HERMES_PIN_BOOT 5
#endif
#ifndef HERMES_PIN_PA_EN
#define HERMES_PIN_PA_EN 8
#endif
...
// ---- 弹簧触摸键（IO5，S3 原生触摸外设）----
// 通道映射（soc/touch_sensor_channel.h）：T0 为内部去噪通道（无外部引脚），
// TOUCH_PAD_NUMn = GPIOn（n=1..14），故 IO5 = TOUCH_PAD_NUM5。
// 历史教训：曾误用 NUM4（=GPIO4=I2S_DO），触摸外设周期充放电劫持音频数据线
// 导致喇叭噪音且弹簧无效；且 S3 的 raw 方向与经典 ESP32 相反——S3 固定充放电
// 次数、记录期间 8MHz 时钟周期数，触摸→电容增大→值增大（见 driver 注释）。
constexpr touch_pad_t kTouchPadKey = TOUCH_PAD_NUM5;
// 实测（本板弹簧电极）：单指 pct≈+7、掌心 +10、静息 0 —— 触摸幅度偏弱。
// 阈值取最小触摸幅度约一半（+4%），并加迟滞：低于 +2% 才判松开，防边界抖动。
constexpr uint32_t kTouchPressPercent = 4;    // 平滑值高于基线 4% 判按下
constexpr uint32_t kTouchReleasePercent = 2;  // 已按下时低于 2% 才判松开（迟滞）
constexpr uint32_t kTouchDebugLogMs = 0;      // 调试打印间隔（ms），0 关闭（调试时改 500）
uint32_t touchBaseline = 0;
uint32_t touchSmoothed = 0;
bool touchKeyReady = false;

void initTouchKey() {
  // 初始化序列对齐 Arduino core esp32-hal-touch.c 的 SOC_TOUCH_VERSION_2 分支
  //（Espressif 官方验证的 S2/S3 可用序列）。
  if (touch_pad_init() != ESP_OK) return;
  ...
  // 去噪通道 T0 与被测通道同步工作，扣除电源/EMI 共模干扰。
  touch_pad_denoise_t denoise = {TOUCH_PAD_DENOISE_BIT4, TOUCH_PAD_DENOISE_CAP_L4};
  touch_pad_denoise_set_config(&denoise);
  touch_pad_denoise_enable();
  touch_pad_set_fsm_mode(TOUCH_FSM_MODE_TIMER);  // 定时器模式自动连续采样
  touch_pad_fsm_start();
  if (touch_pad_config(kTouchPadKey) != ESP_OK) return;  // 含 IO 到 RTC 功能的路由
  delay(50);  // touch_pad_config 后需等一个测量周期才有有效读数
  // 上电校准基线：16 次采样取平均（此时尚无人触摸）。
  uint32_t sum = 0;
  uint8_t samples = 0;
  for (uint8_t i = 0; i < 16; ++i) {
    uint32_t raw = 0;
    if (touch_pad_read_raw_data(kTouchPadKey, &raw) == ESP_OK && raw > 0) {
      sum += raw;
      ++samples;
      touchSmoothed = raw;
    }
  ...
  if (samples < 8) {
    Serial.println(F("Touch key init failed: no samples"));
    return;
  }
  touchBaseline = sum / samples;
  touchKeyReady = touchBaseline > 0;
  Serial.printf("Touch key ready pad=%d gpio=%d baseline=%u pressIfAbove=%u releaseIfBelow=%u\n",
                static_cast<int>(kTouchPadKey), TOUCH_PAD_NUM5_GPIO_NUM,
                static_cast<unsigned>(touchBaseline),
                static_cast<unsigned>(touchBaseline * (100 + kTouchPressPercent) / 100),
                static_cast<unsigned>(touchBaseline * (100 + kTouchReleasePercent) / 100));
}

// 读取触摸状态：raw 经 1/4 IIR 平滑后与基线比较（S3：触摸→电容增大→值增大）。
// 注意：kTouchDebugLogMs 是 constexpr 变量，不能用 #if（预处理器把未定义标识符
// 当 0，上一版 #if 写法把调试输出整个编译掉了），必须用普通 if（编译器消除死分支）。
bool touchKeyPressed() {
  if (!touchKeyReady) return false;
  uint32_t raw = 0;
  if (touch_pad_read_raw_data(kTouchPadKey, &raw) != ESP_OK || raw == 0) {
    static uint32_t lastReadErrMs = 0;
    uint32_t errNow = millis();
    if (kTouchDebugLogMs > 0 && errNow - lastReadErrMs >= 5000) {
      lastReadErrMs = errNow;
      Serial.println(F("[touch] read failed"));
    }
    return false;
  }
  touchSmoothed = (touchSmoothed * 3 + raw) >> 2;
  // 迟滞判定：未按下需超 +4% 才触发；已按下需回落到 +2% 以下才释放。
  static bool touchLatch = false;
  uint32_t pressLevel = touchBaseline * (100 + kTouchPressPercent) / 100;
  uint32_t releaseLevel = touchBaseline * (100 + kTouchReleasePercent) / 100;
  touchLatch = touchLatch ? (touchSmoothed > releaseLevel) : (touchSmoothed > pressLevel);
  bool pressed = touchLatch;

  if (kTouchDebugLogMs > 0) {
    static uint32_t lastTouchLogMs = 0;
    static bool lastLoggedPressed = false;
    uint32_t now = millis();
    // 周期打印 + pressed 边沿（按下/松开）立即打印，实时观察触摸反应。
    if (now - lastTouchLogMs >= kTouchDebugLogMs || pressed != lastLoggedPressed) {
      lastTouchLogMs = now;
      lastLoggedPressed = pressed;
      long diff = static_cast<long>(touchSmoothed) - static_cast<long>(touchBaseline);
      int pct = static_cast<int>(diff * 100 / static_cast<long>(touchBaseline));
      Serial.printf("[touch] raw=%u smooth=%u base=%u pct=%d pressed=%d\n",
                    static_cast<unsigned>(raw), static_cast<unsigned>(touchSmoothed),
                    static_cast<unsigned>(touchBaseline), pct, pressed ? 1 : 0);
    }
  }

  // 慢速基线漂移自适应：每 1s 一次、仅未按下且偏差 <5% 时以 1/32 权重跟随温漂，
  // 绝不追逐真实触摸（触摸偏差远大于 5%，且按住期间禁止跟随防假松开）。
  static uint32_t lastBaselineAdjMs = 0;
  uint32_t adjNow = millis();
  if (adjNow - lastBaselineAdjMs >= 1000) {
    lastBaselineAdjMs = adjNow;
    long dev = static_cast<long>(touchSmoothed) - static_cast<long>(touchBaseline);
    if (dev < 0) dev = -dev;
    if (!touchLatch && dev * 20 < static_cast<long>(touchBaseline)) {
      touchBaseline = (touchBaseline * 31 + touchSmoothed) / 32;
    }
  }
  return pressed;
}

#define HERMES_BOOT_PRESSED() touchKeyPressed()
...
// ST7789 240x240 彩屏：原生 RGB565 帧缓冲直接绘制（参考 esp-sparkbot/v1）。
// 中文 MCU 字幕例外：由 U8g2 在 128x64 单色画布排版后按区域放大搬运。
constexpr int kLcdOutWidth = 240;
constexpr int kLcdOutHeight = 240;
// 配色（RGB565），与 esp-sparkbot/v1 一致。
...
// 触摸键手指点击间隔比机械键慢：双击窗口取 450ms（同 esp-sparkbot/v1 触摸键）。
constexpr uint32_t kBootDoubleClickMs = 450;
...
// U8g2 仅作为 128x64 单色画布的字体渲染引擎（空字节回调，不驱动任何总线），
// 渲染结果经 lcdPresent() 放大搬运到 ST7789。
// 注意：不能用 U8G2_NULL——它内部的"显示缓冲"只有 8 字节（u8g2_Setup_null
// 里的 static uint8_t buf[8]），而 oledClearBuffer()/oledSetPixel() 按
// 128x64 页式布局操作 1024 字节，越界写入会把 .bss 相邻全局对象清零
// （曾把 TFT_eSPI 的 SPIClass::paramLock 清成 NULL，触发
// assert xQueueSemaphoreTake queue.c 崩溃循环）。这里改用 SSD1306
// 128x64 全帧缓冲（1024 字节，vertical_top_lsb 页式布局与 oledSetPixel
// 的位序假设一致）+ 空传输回调自行搭建。
class U8g2NullCanvas : public U8G2 {
 public:
  explicit U8g2NullCanvas(const u8g2_cb_t *rotation) : U8G2() {
    u8g2_Setup_ssd1306_128x64_noname_f(&u8g2, rotation, u8x8_byte_empty, u8x8_dummy_cb);
  }
};
U8g2NullCanvas oledTextRenderer(U8G2_R0);
TFT_eSPI tft;
uint16_t *lcdFrame = nullptr;
...
// 本板 LCD_BL（GPIO2）为低电平点亮（与 S3 时钟 v1.4 固件的
// analogWrite(LCD_BL_PIN, 255 - 亮度) 反相逻辑一致），开关极性不能反。
bool lcdDisplayOn() {
...
void leaveIdlePowerSave() {
  ...
  WiFi.setSleep(false);
  if (oledReady && !lcdDisplayOn()) {
    Serial.println(F("Idle power save wake: LCD display-on failed"));
  }
  oledDirty = true;
...
void tickIdlePowerSave() {
  ...
  bool bootPressed = HERMES_BOOT_PRESSED();
  ...
  if (oledReady && !lcdDisplayOff()) {
    Serial.println(F("Idle power save: LCD display-off failed"));
  }
...
bool lcdEnsureFrame() {
  if (lcdFrame) return true;
  lcdFrame = static_cast<uint16_t *>(calloc(static_cast<size_t>(kLcdOutWidth) * kLcdOutHeight, sizeof(uint16_t)));
  return lcdFrame != nullptr;
}

void lcdClearFrame() {
  if (!lcdFrame) return;
  size_t total = static_cast<size_t>(kLcdOutWidth) * kLcdOutHeight;
  for (size_t i = 0; i < total; ++i) lcdFrame[i] = kColorBg;
}

void lcdSetPixel(int x, int y, uint16_t color) {
  if (!lcdFrame || x < 0 || x >= kLcdOutWidth || y < 0 || y >= kLcdOutHeight) return;
  lcdFrame[y * kLcdOutWidth + x] = color;
}
...
void lcdDrawCenteredText(int y, const String &text, uint8_t scale, uint16_t color) {
  int x = (kLcdOutWidth - lcdTextWidth(text, scale)) / 2;
...
// 把 U8g2 单色画布（128x64）的 [srcY0, srcY0+srcH) 行区域放大搬运到
// RGB565 主帧：横向 1.875x 填满 240 宽，纵向缩放到 dstH 高、起于 dstY。
void blitOledRegionToLcd(int srcY0, int srcH, int dstY, int dstH, uint16_t color) {
  if (!lcdFrame) return;
  for (int dy = 0; dy < dstH; ++dy) {
    int sy = srcY0 + dy * srcH / dstH;
    if (sy < 0 || sy >= kOledHeight) continue;
    const uint8_t *page = oledBuffer + (sy >> 3) * kOledWidth;
    uint8_t bit = 1 << (sy & 7);
    uint16_t *dst = lcdFrame + (dstY + dy) * kLcdOutWidth;
    for (int dx = 0; dx < kLcdOutWidth; ++dx) {
      int sx = dx * kOledWidth / kLcdOutWidth;
      if (page[sx] & bit) dst[dx] = color;
    }
  }
}
...
void drawMcuSubtitle() {
  ...
  // 字幕排版区域（y16..63）搬运到屏幕中部 128px 高的区域。
  blitOledRegionToLcd(16, 48, 48, 128, kColorFg);
}
...
void drawBatteryGlyph(uint8_t x, uint8_t y, uint16_t color) {
#if HERMES_ENABLE_BATTERY_ADC
  updateBatteryReading();
  lcdDrawFrame(x, y + 4, 30, 14, color);
  lcdDrawBox(x + 30, y + 8, 4, 6, color);
  uint8_t fill = static_cast<uint8_t>((batteryLevelPercent * 22UL) / 100UL);
  if (fill > 0) {
    lcdDrawBox(x + 4, y + 8, fill, 6, color);
  }
#else
  // 外接电源：显示 AC 插头字样（scale2，两字符宽 22px，区域内居中）。
  lcdDrawText(x + 4, y + 6, F("AC"), 2, color);
#endif
}

void drawTopStatusBar() {
  lcdDrawText(6, 6, F("HSTUDIO"), 2, kColorFg);
  drawBatteryGlyph(158, 4, kColorFg);
  drawWifiGlyph(196, 4, kColorFg);
  lcdDrawHLine(0, 28, kLcdOutWidth, kColorMuted);
}

void drawEye(int x, int y, bool blink, bool thinking, bool error) {
  const int w = 56;
  const int h = thinking ? 44 : 56;
  if (error) {
    lcdDrawBox(x + 10, y + h / 2 - 6, w - 20, 6, kColorWarn);
    lcdDrawBox(x + 14, y + h / 2 + 4, w - 28, 6, kColorWarn);
    return;
  }
  if (blink) {
  ...
  lcdDrawSoftBox(x, y, w, h, 14, kColorFg);
  int phase = static_cast<int>((millis() / 1400UL) % 5);
  int lookX = thinking ? 0 : (phase == 1 ? -5 : (phase == 3 ? 5 : 0));
  lcdDrawSoftBox(x + w / 2 - 10 + lookX, y + h / 2 - 10, 20, 20, 6, kColorBg);
}
...
void drawInteractionFrame() {
  ...
  if (showSubtitle) {
    ...
    lcdDrawCenteredText(50, fitOledText(title, 12), 3, titleColor);
    ...
    if (detail.length() > 20 || mcuInteractionStatus == F("failed")) {
      ...
      lcdDrawCenteredText(120, fitOledText(detail, 20), 2, kColorFg);
    }
  }
...
bool lcdPresent() {
  if (!lcdEnsureFrame()) return false;
  // 注意：setAddrWindow(x, y, w, h) 第 3/4 参数是"宽高"，不是右下角坐标
  // （传坐标的旧 API 是 setWindow(x0,y0,x1,y1)）。之前误传 (239,239) 使
  // 实际窗口变成 239x239，而 pushPixels 推 240x240 数据，地址计数器每
  // 239 像素换行 → 每行左移 1px 形成平行四边形斜拉，第 120 行累计错位
  // 恰好半行宽 → 左右两半画面互换。
  tft.setAddrWindow(0, 0, kLcdOutWidth, kLcdOutHeight);
  tft.pushPixels(lcdFrame, kLcdOutWidth * kLcdOutHeight);
  return true;
}

bool oledFlushPages(uint8_t firstPage, uint8_t lastPage) {
  // ST7789 SPI 整帧输出足够快，无需 SSD1306 式分页；保留签名兼容调用处。
  (void)firstPage;
  (void)lastPage;
  return lcdPresent();
}
...
void drawOledFrame() {
  ...
  if (thinking) {
    ...
    lcdDrawFrame(20, 180, 200, 8, kColorMuted);
    uint8_t filled = static_cast<uint8_t>((oledProgress * 192) / 100);
    if (filled > 0) lcdDrawBox(24, 182, filled, 4, kColorAccent);
  }
...
void initLcdDisplay() {
  // I2C 总线仍需初始化（ES8311 codec 挂在同一总线）。
  Wire.begin(kPinI2cSda, kPinI2cScl);
  Wire.setClock(kCodecI2cClockHz);
  delay(40);
  // ST7789 为 SPI 直驱（TFT_eSPI），无地址探测；引脚来自编译宏。
  tft.init();
  tft.setRotation(0);
  tft.fillScreen(TFT_BLACK);
  if (TFT_BL >= 0) {
    pinMode(TFT_BL, OUTPUT);
    // 背光低电平点亮：tft.init() 内部按 TFT_BL_TURNING_ON_LEVEL(已定义为 0)
    // 拉低过一次，这里显式再拉低，确保亮屏。
    digitalWrite(TFT_BL, LOW);
  }
  oledFound = true;
  oledReady = lcdEnsureFrame();
  Serial.printf("LCD ready mosi=%d sclk=%d dc=%d rst=%d bl=%d\n", TFT_MOSI, TFT_SCLK,
                TFT_DC, TFT_RST, TFT_BL);
  oledClearBuffer();
  lcdClearFrame();
  oledReady = oledFlush();
  if (oledReady) setOledStatus(OledMode::Boot, F("BOOT"), F("LCD ONLINE"), 10);
}
...
bool shouldInterruptAudioForVoice() {
  bool pressed = HERMES_BOOT_PRESSED();
  uint32_t now = millis();
...
// 喇叭自检：本地直接推 1kHz 正弦（完全绕过网络/协议/TTS 数据层），
// 用于切分"模拟输出链路故障"（无声 → PA/codec/I2S 问题）与"TTS 数据问题"
//（有声 → 查服务器音频流）。开机音频初始化成功后自动播放一次。
void playSpeakerSelfTestBeep() {
  if (!i2sReady || !es8311Ready) return;
  setPowerAmp(true);
  es8311UpdateBits(0x31, 0x60, 0x00);
  setI2sSampleRate(kAudioSampleRate);
  Serial.println(F("Speaker self-test beep (1kHz 350ms)"));
  const uint32_t totalFrames = static_cast<uint32_t>(kAudioSampleRate) * 350UL / 1000UL;
  constexpr size_t kChunkFrames = 256;
  uint8_t buffer[kChunkFrames * 4];  // 16bit 立体声帧
  uint32_t frame = 0;
  while (frame < totalFrames) {
    uint32_t n = totalFrames - frame;
    if (n > kChunkFrames) n = kChunkFrames;
    for (uint32_t i = 0; i < n; ++i) {
      float phase = 2.0f * 3.14159265f * 1000.0f * static_cast<float>(frame + i) /
                    static_cast<float>(kAudioSampleRate);
      int16_t sample = static_cast<int16_t>(10000.0f * sinf(phase));
      size_t off = i * 4;
      uint8_t lo = static_cast<uint8_t>(sample & 0xFF);
      uint8_t hi = static_cast<uint8_t>((sample >> 8) & 0xFF);
      buffer[off] = buffer[off + 2] = lo;
      buffer[off + 1] = buffer[off + 3] = hi;
    }
    size_t written = 0;
    if (i2s_write(kI2sPort, buffer, n * 4, &written, pdMS_TO_TICKS(1000)) != ESP_OK) break;
    frame += n;
  }
  delay(400);  // 等 DMA 排空再清缓冲，防止尾音被切
  i2s_zero_dma_buffer(kI2sPort);
}
...
void initAudioHardware() {
  ...
  // 自检哔声已取消（如需恢复，取消下一行注释即可）
  // if (i2sReady && es8311Ready) playSpeakerSelfTestBeep();
}
...
String mcuStatusJson() {
  ...
  json += F("\",\"batteryKnown\":");
#if HERMES_ENABLE_BATTERY_ADC
  json += F("true,\"batteryLevel\":");
  json += batteryLevelPercent;
  json += F(",\"batteryVoltageMv\":");
  json += batteryVoltageMv;
#else
  json += F("false,\"batteryLevel\":0,\"batteryVoltageMv\":0");
#endif
  json += F("}");
  return json;
}
...
bool playPcmStereoStream(WiFiClient *stream, int contentLength, uint32_t sampleRate) {
  ...
  while (prebufferOffset < prebufferLength || remaining != 0) {
    ...
    // 诊断：统计已播放数据的峰值幅度（音量缩放后的值）。
    for (size_t i = 0; i + 1 < alignedBytes; i += 2) {
      int16_t s = static_cast<int16_t>(static_cast<uint16_t>(buffer[i]) |
                                       (static_cast<uint16_t>(buffer[i + 1]) << 8));
      int16_t m = s < 0 ? static_cast<int16_t>(-s) : s;
      if (m > peakSample) peakSample = m;
    }
    size_t written = 0;
  ...
  lastAudioDetail = String(F("pcm played bytes=")) + String(playedBytes) +
                    ...
                    F(", peak=") + String(peakSample) +
                    F(", padded=") + String(paddedBytes);
  Serial.println(lastAudioDetail);
  audioBusy = false;
...
bool connectSavedWifi() {
  ...
  // 兜底：扫描可能漏检（信号弱、频段差异等），此时仍直接尝试连接已保存的
  // SSID。WiFi.begin 会主动探测目标网络，不依赖本次扫描结果，避免设备因
  // 误判「网络不在」而进入配网 AP 模式，导致局域网 IP 不可达。
  for (int i = 0; i < count; ++i) {
    if (scannedNetworkHasSsid(ssids[i])) continue;  // 已在上方尝试过
    Serial.printf("Saved WiFi not in scan, still trying direct connect ssid=%s\n", ssids[i].c_str());
    esp_rom_printf("Saved WiFi not in scan, still trying direct connect ssid=%s\n", ssids[i].c_str());
    if (connectWifiCredentials(ssids[i], passes[i], WIFI_STA)) {
      rememberWifiCredential(ssids[i], passes[i]);
      return true;
    }
...
void handleRoot() {
  if (wifiReady && WiFi.status() == WL_CONNECTED) {
    // 首页只渲染缓存的状态页，不触发同步的局域网/远程扫描，否则访问首页
    // 会被 scanLanDevices()（最长 kLanDiscoveryTimeoutMs）+ 远程 SSL 发现阻塞，
    // 导致 Web 页长时间无响应、看起来无法访问。
    sendStatusPage();
    return;
  }
  sendWifiPage();
...
// WiFi 掉线重连或切换模式后，之前 server.begin() 建立的监听 socket 会被
// WiFi.mode/WiFi.disconnect 破坏。这里重新 begin 以恢复 WebServer 监听，
// 否则设备出站正常但局域网入站（Web 页 / HTTP）无法访问。
...
void loop() {
  ...
  if (wifiReady && WiFi.status() != WL_CONNECTED) {
    ...
    if (wifiDisconnectedSinceMs == 0) {
      ...
      // 重连过程会切换 WiFi 模式/断开，破坏了之前的监听，需重建 WebServer。
      restartHttpServer();
    }
  } else if (wifiReady) {
    wifiDisconnectedSinceMs = 0;
