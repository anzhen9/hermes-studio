...
export async function fetchLocalPets(): Promise<LocalImportedPet[]> {
  const res = await request<{ pets: LocalImportedPet[] }>('/api/studio/pets/local')
  return res.pets
}

export function localPetAssetUrl(slug: string): string {
  return `/api/studio/pets/local/${encodeURIComponent(slug)}/asset`
}

export function localPetPreviewUrl(slug: string): string {
  return `/api/studio/pets/local/${encodeURIComponent(slug)}/preview`
}
...
export async function importLocalPet(input: {
  ...
  const res = await request<{ pet: LocalImportedPet }>('/api/studio/pets/import', {
    method: 'POST',
    body: form,
  })
  return res.pet
...
export async function deleteLocalPet(slug: string): Promise<{ deleted: boolean; wasActive: boolean }> {
  return request<{ deleted: boolean; wasActive: boolean }>(`/api/studio/pets/local/${encodeURIComponent(slug)}`, {
    method: 'DELETE',
  })
}
